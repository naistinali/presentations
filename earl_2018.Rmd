...
We wanted functionality to organize and keep track of all our experiments
with different models, preprocessing options etc. Unfortunately **caretEnsemble** did not meet
our needs.
...
1. Set settings that will apply to all models in the 'model grid' by default.
2. Add individual model specifications to the model grid.
3. Train models in the model grid.

Everything is (shared setttings, indvidual model specifications and fitted model)
is saved in one object - the model grid.
...
Since data preprocessing is an integral part of the model development, it is obvious
to include this as part of models spanned by a model grid.

To do this we recommend leveraging a model grid with the **recipes** package.

With **recipes** the specification of model terms and preprocessing steps can be enumerated 
sequentially. The recipe can be estimated and applied to any dataset. 
